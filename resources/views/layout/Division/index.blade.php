        ...
        <div class="relative overflow-x-auto shadow-md sm:rounded-lg px-3">
            <div class="flex items-center justify-center mb-5 ">
                <div class="w-full p-3">
                    <div class="px-4">
                        <h1 class="text-2xl font-bold antialiased pb-3 pt-6 text-green-600">Liste des divisions</h1>
                    </div>
                        ...
                        @foreach ($divisions as $division)
                            <div id="card-body" class="w-full max-w-sm border border-gray-200 rounded-lg hover:shadow-lg">
                                <div class="flex justify-end px-4 pt-2">
                                    <!-- Dropdown Button -->
                                    <button id="dropdownButton{{ $division->id_division }}"
                                        data-dropdown-toggle="dropdown{{ $division->id_division }}"
                                    ...
                                    <!-- Dropdown Menu -->
                                    <div id="dropdown{{ $division->id_division }}"
                                        class="z-10 hidden text-base list-none bg-white divide-y divide-gray-100 rounded-lg shadow-xl w-44">
                                        <ul class="py-2" aria-labelledby="dropdownButton{{ $division->id_division }}">
                                            <li>
                                                <a href="{{ url('/divisions/' . $division->id_division . '/edit') }}"
                                                    class="block px-4 py-2 text-sm text-green-700 hover:bg-gray-100">Modifier</a>
                                            </li>
                                            <li>
                                                <form method="post"
                                                    action="{{ url('/divisions/' . $division->id_division) }}">
                                                    @csrf
                                                    @method('DELETE')
                                ...
                                <div class="flex flex-col items-center pb-4">
                                    <h5 class="text-xl font-medium text-center text-gray-900 mb-2">
                                        {{ $division->lable }}
                                    </h5>
                                    <div class="flex mt-4 space-x-3 md:mt-6">
                                        <a href="{{ url('/divisions/' . $division->id_division) }}"
